 ...
 * @since 1.0
 */
abstract class MyActiveRecord extends CModel
{
...
class CBaseActiveRelation extends CComponent
{
     ...
     */
    private static $dbfgmanage = null;
 
    protected static function getAdvertDbConnection()
    {
        if (self::$dbfgmanage !== null)
            return self::$dbfgmanage;
        else
        {
            self::$dbfgmanage = Yii::app()->dbfgmanage;
            if (self::$dbfgmanage instanceof CDbConnection)
            {
                self::$dbfgmanage->setActive(true);
                return self::$dbfgmanage;
            }
            else
                throw new CDbException(Yii::t('yii','Active Record requires a "db" CDbConnection application component.'));
